 ...
 *
 * Telemetry: every dispatch is wrapped in `withRun`, which writes a worker_runs
 * row before the worker starts and closes it after. `dispatch` therefore returns
 * PLAIN objects, not NextResponse — a NextResponse in worker_runs.result would
 * serialise to nothing useful. GET does the NextResponse.json() wrapping.
 */
...
/**
 * The dispatchable `?worker=` keys. This exact string set is the join key
 * between worker_runs, blueprint.ts's BlueprintNode.worker and this route; a
 * typo would unhook a node's telemetry silently, so a test asserts it matches
 * CRON_WORKER_KEYS in blueprint.ts.
 */
const VALID_WORKERS = [
...
/**
 * The worker dispatch chain. Returns the plain response payload, or null for a
 * worker this chain does not handle (which GET turns into the 400).
 */
async function dispatch(
    ...
    if (worker === 'republish-social') {
        ...
        return { success: results.every(r => r.success), worker: 'republish-social', results };
    }
    ...
    if (worker === 'diag-image-reel') {
        ...
        if (!buf) {
            return { success: false, stage: 'fetch', error: 'image fetch returned null' };
        }
    ...
    if (worker === 'health-monitor') {
        ...
        // Persist the WHOLE snapshot. `fireHealthAlertsIfChanged` only stores a
        // {key: level} map in worker_locks for alert dedup — no detail, no
        // actionable, no checkedAt — so before this nothing anywhere held a
        // snapshot the diagnostics panel could read. engine_config has NO
        // auto-update trigger on updated_at, so it is set explicitly or the
        // consumer's "age" reading is silently wrong. Best-effort: a bookkeeping
        // failure must never fail the health cron.
        try {
            const { error } = await supabaseAdmin.from('engine_config').upsert(
                { key: 'health_snapshot', value: snap, updated_at: new Date().toISOString() },
                { onConflict: 'key' },
            );
            // supabase-js returns errors rather than throwing, so both paths
            // need handling or a failed write would look like a success.
            if (error) console.error('[Cron] health-monitor: could not persist snapshot:', error.message);
        } catch (e) {
            console.error('[Cron] health-monitor: could not persist snapshot (continuing):', e);
        }
    ...
    if (worker === 'newsletter') {
        ...
        try {
            ...
            if (itemCount === 0) {
                return { success: true, worker: 'newsletter', skipped: 'no items this week' };
            }
...
export async function GET(req: NextRequest) {
    ...
    // Unknown workers are rejected BEFORE withRun, so a typo'd or probed
    // ?worker= never creates a worker_runs row and cannot pollute the map.
    if (!worker || !DISPATCHABLE.has(worker)) {
    ...
    // Query-param validation, hoisted out of the dispatch chain for the same
    // reason: a missing ?postIds= is a caller mistake, not a worker run.
    if (worker === 'republish-social' && !(searchParams.get('postIds') || searchParams.get('postId'))) {
        return NextResponse.json({ error: 'postIds query param is required' }, { status: 400 });
    }
    if (worker === 'render' && !(searchParams.get('postIds') || searchParams.get('postId'))) {
        return NextResponse.json({ error: 'postIds query param is required' }, { status: 400 });
    }
    if (worker === 'diag-image-reel' && !searchParams.get('url')) {
        return NextResponse.json({ error: 'url query param required' }, { status: 400 });
    }

    try {
        // withRun records the run and RE-THROWS on failure, so the catch below
        // still runs and still returns its 500 — the failure is merely recorded
        // first.
        const payload = await withRun(worker, () => dispatch(worker, searchParams));
