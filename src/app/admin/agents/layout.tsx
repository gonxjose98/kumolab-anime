...
export default async function AdminSubLayout({ children }: { children: React.ReactNode }) {
    ...
    if (!session) { redirect('/admin/login'); return null; }
            ...
            <header className="admin-header px-5 py-3 flex justify-between items-center backdrop-blur-2xl sticky top-0 z-50 transition-all duration-300" style={{ background: 'var(--surface-color)', borderBottom: '1px solid var(--border-subtle)' }}>
                <Link href="/admin/dashboard" className="flex items-center gap-3 hover:opacity-80 transition-opacity group">
                    ...
                    <div className="flex flex-col">
                        <span className="text-sm font-bold tracking-tight" style={{ fontFamily: 'var(--font-display)', background: 'linear-gradient(135deg, #00d4ff, #7b61ff)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>KUMOLAB</span>
                        <span className="text-[8px] font-bold uppercase tracking-[0.3em]" style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-display)' }}>Admin Console</span>
                    </div>
                </Link>
                <div className="flex items-center gap-3">
                    <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <span className="text-[10px] font-mono" style={{ color: 'var(--text-tertiary)' }}>{session.user.email}</span>
                    </div>
                    <HamburgerMenu />
                    <LogoutButton />
                </div>
