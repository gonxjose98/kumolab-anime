...
const fmtInt = (n: number | null | undefined) => (n == null ? '—' : Math.round(n).toLocaleString('en-US'));
const fmtPct = (n: number | null | undefined) => (n == null ? '—' : `${n}%`);
const fmtMoney = (n: number | null | undefined, ccy = 'USD') =>
...
// Each platform is its OWN card with a colored accent + label, so it's always
...
function getVal(row: MonthlyReportRow | null, path: string): number | null {
    if (!row) return null;
...
function fmtVal(v: number | null, fmt: Fmt | undefined, ccy: string): string {
    switch (fmt) {
